...
VALUES (
  ...
  'Tier lists are easy to grade with hindsight and hard to make before the season starts. Creators were working from PTR testing, unfinished tuning, a new dungeon pool, and no live-key evidence. Five weeks into Midnight Season 2, we finally have enough data to ask a fairer question: whose full pre-season ordering looks most like the high-key meta that actually formed?
...
The result is not a dunk list. Predicting Blood Death Knight, Holy Paladin, Arms Warrior, and Arcane Mage near the top was the easy part in retrospect. Predicting the exact order through 40 specs, before live keys existed, was not.

> Eligibility: the forecast had to be published before Season 2 keys opened, explicitly cover Mythic+, and rank at least 26 of the 40 specs. A complete 26- or 27-spec DPS board counts as “most specs.” Short six- or seven-spec tank and healer lists remain useful source material, but they are not placed on this leaderboard.
...
izen finished first at 61.6 and was the most even full-board forecaster, scoring 60.0 for tanks, 61.9 for healers, and 63.0 for DPS. YoDaTV was only one point behind after the coverage adjustment and made the strongest tank prediction in the group at 73.3. Petko and Tactyks / Method followed with balanced full-roster results. Tettles had a 60.4 raw DPS score, then received the same small coverage adjustment applied to every DPS-only board.
...
1. **Keep the creator''s own tiers and ties.** Specs in the same tier stay tied.
2. **Normalize the prediction inside each role.** The top predicted position becomes 100, the bottom becomes 0, and tied specs share the midpoint of the positions they occupy.
3. **Build the actual five-week ordering.** Each spec''s MythicStats representation is averaged across periods 1077 to 1081, then converted to the same within-role 0 to 100 scale.
4. **Measure every miss.** Per-spec error is the absolute difference between the prediction percentile and the actual percentile.
5. **Score each role.** Role accuracy is `max(0, 100 - 2 × mean absolute error)`.
6. **Weight roles equally.** A full board''s tank, healer, and DPS scores each contribute one-third. The 27 DPS specs cannot drown out the smaller tank and healer pools.
7. **Apply a small coverage adjustment.** Final accuracy is `raw agreement × [0.85 + 0.15 × (ranked specs ÷ 40)]`.
...
## Notes on the 12 qualifying forecasts
...
- **Tettles** ranked all 27 DPS specs and placed fifth after the coverage adjustment.
...
This is also an early-season checkpoint. Later tuning can make a week-five forecast look better or worse. The result answers one narrow question: **whose substantial pre-season Mythic+ ordering most closely resembled the first five weeks of the high-key meta?**
