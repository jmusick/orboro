...
VALUES (
  '9ae2eebf-aac1-4a73-a78e-cb45d7d858f1',
  'why-the-best-great-vault-reward-in-midnight-season-2-isnt-an-item',
  'Why the Best Great Vault Reward in Midnight Season 2 Isn''t an Item',
  'In Midnight Season 2, the most valuable reward in your Great Vault will often be the one that is not a piece of gear. If you are trying to build toward long-term best-in-slot equipment, the **Nebulous Voidcore** should be your default weekly choice.

That does not mean the Voidcore is correct for every character in every possible Vault. A tier piece that completes your four-piece bonus or a weapon needed for progression can be worth taking immediately. But Season 2 gives the Voidcore an unusual combination of targeting, duplicate protection, and future value that an ordinary Vault item cannot match.

The short version is simple:

> **If taking a Vault item would not make you stop bonus rolling the boss or dungeon that drops it, take the Voidcore instead.**
...
The reward is also aligned with the Great Vault level for that content rather than its ordinary end-of-run drop:

| Content | Bonus-roll reward |
| --- | --- |
| Heroic raid boss | Myth 1/6 |
| Mythic raid bosses 1–6 | Myth 6/6 |
...
## The Knockout List Changes the Math

The real value of a Voidcore is not simply that it produces another random item. Bonus rolls use a knockout system.

When a Voidcore awards an item, that item is recorded for your character and removed from subsequent bonus rolls from that loot pool. Each roll therefore improves the odds of the next one. Blizzard described this intended behavior when it explained and fixed the initial Season 1 duplicate bug: [each Voidcore item is checked off a character-specific list](https://us.forums.blizzard.com/en/wow/t/bonus-rolls-%E2%80%93-what-happened-and-next-steps/2296587/1).
...
That is why an exciting Vault item can still be the less efficient choice.

Imagine that a boss has four items available to your loot specialization and you want all four.

- Taking four consecutive Voidcores and rolling that boss four times gives you all four items. Each roll removes one option.
- Taking one of those items from the Vault and then using three Voidcores still leaves all four items in the bonus-roll pool.
...
The guaranteed Vault item feels safer in the moment, but the Voidcore makes the entire sequence more deterministic.

## Why Heroic Raid Bosses Are So Valuable
...
## Claim Every Week Does Not Mean Spend Every Week

The optimal habit is to **claim** a Voidcore from every eligible Vault. You do not have to spend it immediately.

Banking a core preserves its option value. You can wait until your group reaches Heroic, until an early Mythic boss becomes realistically farmable, or until your normal drops reveal which target pool has the greatest remaining value.
...
Changing loot specialization can sometimes remove an unwanted weapon or trinket from the available list. That can turn an average roll into a much stronger one, but the correct specialization varies by class.

## When You Should Take the Vault Item

The Voidcore is a default, not an inflexible commandment. Take the actual item when its immediate or guaranteed value is greater than the long-term value of another roll.
...
## The Best Vault Reward Is Often Future Control

The Great Vault normally asks you to choose the best item from a random collection. The Nebulous Voidcore changes that decision. Instead of taking one result now, you can choose the activity, difficulty, loot specialization, and timing of a future result—and improve the odds of every roll that follows.

That control is why the Voidcore is usually the optimal Season 2 choice. If you intend to keep rolling a source, taking one of its items from the Vault does not save you a roll. It creates a chance that a future roll gives you the same item again.

Claim the core, build a target plan, and let the knockout list do the work. Just remember that a character''s real progression needs can still outweigh the cleanest spreadsheet answer.',
  'post',
  'published',
...
SELECT
  (SELECT id FROM content WHERE slug = 'why-the-best-great-vault-reward-in-midnight-season-2-isnt-an-item'),
  id
FROM categories
